...
=======
                    @if (($role ?? '') === 'admin')
>>>>>>> 7972553dc1ea7933c204bf0b43cc4a8ed6689dbf
                    <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 {{ Request::routeIs('admin.dashboard') ? 'bg-violet-500/[0.12] dark:bg-violet-500/[0.24]' : 'bg-[linear-gradient(135deg,var(--tw-gradient-stops))]' }}">
                        <a class="block text-gray-800 dark:text-gray-100 truncate transition" href="{{ route('admin.dashboard') }}">
                            <div class="flex items-center">
                                <svg class="shrink-0 fill-current {{ Request::routeIs('admin.dashboard') ? 'text-violet-500' : 'text-gray-400 dark:text-gray-500' }}" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                    ...
                    <!-- Pengajuan -->
                    <li x-data="{ open: {{ in_array(Request::segment(1), ['pengajuan']) ? 1 : 0 }}, sidebarExpanded: true }" :class="open ? 'bg-[linear-gradient(135deg,var(--tw-gradient-stops))] from-violet-500/[0.12] dark:from-violet-500/[0.24] to-violet-500/[0.04]' : ''" class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0">
                        <a href="#0" @click.prevent="open = !open; sidebarExpanded = true" :class="!open ? 'hover:text-gray-900 dark:hover:text-white' : ''" class="block text-gray-800 dark:text-gray-100 truncate transition">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <svg :class="open ? 'text-violet-500' : 'text-gray-400 dark:text-gray-500'" class="shrink-0 fill-current" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                        ...
                        <div x-show="open || sidebarExpanded" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="lg:hidden lg:sidebar-expanded:block 2xl:block">
                            <ul class="pl-8 mt-1">
                                <li class="mb-1 last:mb-0">
                                    <a href="{{ route('admin.daftarPengajuan') }}" @click="open = true" :class="{'!text-violet-500': '{{ Route::is('admin.daftarPengajuan') }}'}" class="block text-gray-500/90 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition truncate">
                                ...
                                <li class="mb-1 last:mb-0">
                                    <a href="{{ route('admin.tindakLanjut') }}" @click="open = true" :class="{'!text-violet-500': '{{ Route::is('admin.tindakLanjut') }}'}" class="block text-gray-500/90 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition truncate">
                    ...
                    <!-- riwayat -->
                    <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 {{ Request::routeIs('admin.riwayat') ? 'bg-violet-500/[0.12] dark:bg-violet-500/[0.24]' : 'bg-[linear-gradient(135deg,var(--tw-gradient-stops))]' }}">
                        <a class="block text-gray-800 dark:text-gray-100 truncate transition @if(!in_array(Request::segment(1), ['admin.riwayat'])){{ 'hover:text-gray-900 dark:hover:text-white' }}@endif" href="{{ route('admin.riwayat') }}">
                            <div class="flex items-center">
                                <svg class="shrink-0 fill-current shrink-0 fill-current {{ Request::routeIs('admin.riwayat') ? 'text-violet-500' : 'text-gray-400 dark:text-gray-500' }}" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 512 512">
