    ...
    <div class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1 {{$align === 'right' ? 'right-0' : 'left-0'}}" @click.outside="open = false" @keydown.escape.window="open = false" x-show="open" x-transition:enter="transition ease-out duration-200 transform" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-out duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-cloak>
        ...
        <ul class="notification-container">
            ...
            <li class="border-b border-gray-200 dark:border-gray-700/60 last:border-0">
                ...
                <a class="block py-2 px-4 hover:bg-gray-50 dark:hover:bg-gray-700/50" href="{{ route('admin.daftarPengajuan') }}" @click="open = false" @focus="open = false">
                    @elseif(auth()->user()->role === 'user_opd')
                    <a class="block py-2 px-4 hover:bg-gray-50 dark:hover:bg-gray-700/50" href="{{ route('user_opd.daftarPengajuan') }}" @click="open = false" @focus="open = false">
                        @endif
                        <span class="block text-sm mb-2">📣 <span class="font-medium text-gray-800 dark:text-gray-100">{{ $notification->data['message'] }}</span> <span class="text-xs font-medium text-gray-500 dark:text-gray-500">{{ \Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}</span></span>
                    </a>
            </li>
            @endforeach
        </ul>
